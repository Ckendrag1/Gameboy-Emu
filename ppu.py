...
class PPU:
    ...
    def render_scanline(self):
        ...
        # Always render background first (even if disabled, clear to white)
        if self.lcdc & 0x01:
            self.render_background_line()
        else:
            # Background disabled - fill with palette color 0
            palette_color_0 = self.bgp & 0x03
            for x in range(SCREEN_WIDTH):
                self.screen[self.line][x] = palette_color_0
