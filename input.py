...
class Input:
    ...
    def __init__(self, memory=None, interrupts=None):
        self.memory = memory
        ...
        # NOTE: Arrow keys are primary, WASD is alternative
        self.key_map = {
    ...
    def handle_key_up(self, key):
        ...
        if key in self.key_map:
            ...
            print(f"[Input] Button {button} RELEASED (key: {pygame.key.name(key)})")  # DEBUG
            self.update_p1_register()
            self.update_p1_register()
    ...
    def write_p1(self, value):
        ...
        """
        print(f"[Input] Game reading P1, selecting: buttons={not (value & 0x20)}, dpad={not (value & 0x10)}")  # DEBUG
